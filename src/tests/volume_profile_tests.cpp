...
TEST_F(UriVolumeProfileTest, ParseExample)
{
  ...
  ASSERT_EQ(1u, parsed->count(key));

  csi::VolumeCapability capability = parsed->at(key).capability;
  ...
  Map<string, string> parameters = parsed->at(key).parameters;
  ...
  const vector<string> examples = {
    ...
    R"~({
        "profile" : {
          "volume_capabilities" : {
            "mount" : {
              "mount_flags" : "This should be an array"
            }
          }
        }
      })~",
