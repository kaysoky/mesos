    ...
    add(&Flags::uri,
        ...
        [](const Path& value) -> Option<Error> {
          ...
          if (strings::startsWith(value.string(), "http://")
            ...
            //
            // NOTE: If the scheme is not specified, we assume it is a path.
            if (!value.absolute()) {
...
private:
  ...
  void notify(
      const std::map<std::string, mesos::VolumeProfileAdaptor::ProfileInfo>&
        parsed);
...
public:
  ...
  //
  // TODO(josephw): This currently expects JSON, and mostly performs the parsing
  // manually (because we expect Proto3 instead of Proto2), but we can feasibly
  // take a protobuf as well, if we define one.
  //
  // NOTE: This method is public for testing purposes only.
  static Try<std::map<std::string, mesos::VolumeProfileAdaptor::ProfileInfo>>
